...
class ProductServiceImplTest {
    ...
    void create_with_valid_request() {
        //given
        ProductCreateRequest request = ProductCreateRequest.create(1000, 10, "banana chip", "bla bla", SOLD_OUT);
        //when
        ProductDto productDto = service.create(request);
        //then
        assertThat(productDto).extracting("price", "stock", "name", "detail", "status")
                .containsExactly(1000, 10, "banana chip", "bla bla", SOLD_OUT);
    }
    ...
    @DisplayName("update : 요청 정보의 필드 정보가 null 인 경우, 상품 정보가 수정되면 안된다.")
    @Test
    void update_with_request_that_contains_null() {
        ...
        //when
        ProductDto result = service.update(request);
        //then
        assertThat(result).extracting("name", "price", "stock", "detail", "status").doesNotContainNull();
    }
    ...
    @DisplayName("update : 제고가 0이면 판매 상태는 SELL 이 아니여야한다. 이어야 한다.")
    @Test
    void update_if_stock_is_zero_then_status_is_sold_out(){
        ...
        ProductDto productDto = service.update(req);
        //then
        assertThat(productDto.getStatus()).isNotEqualTo(SELL);
    }
